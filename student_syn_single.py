...
def get_args():
    ...
    parser.add_argument("--dataset_path",
                        type=  str,
                        default = "../../dataset/generated/generate_results_basic_single_objectCIFAR10",
    ...
    parser.add_argument("--model_saving_path",
                        type = str,
                        default = "../../dataset/model_zoo/distilled_student_syn",
    ...
    parser.add_argument('--lambda_kd', type=float, default=0.9, help='trade-off parameter for kd loss')
...
def main(opt: argparse.Namespace):
    ...
    file_handler = logging.FileHandler(os.path.join(opt.logging_saving_path,f"log_{opt.t_model}_{opt.s_model}_{opt.dataset}.txt"),"w+")
    ...
    for epoch in range(1, opt.epoch + 1):
        ...
        time1 = time.time()
        train_acc1, train_acc5, train_loss_cls, train_loss_kd, std, entropy = train_distill(epoch, train_loader, nets, criterions, optimizer, opt)
        time2 = time.time()
        logger.info('epoch {}, total time {:.2f}'.format(epoch, time2 - time1))
        test_acc, test_acc_top5, cls_loss, kd_loss = validate_distill(test_loader, nets, criterions, opt)
        ...
        logger.info("Acc1 %.4f Acc5 %.4f TestClsLoss %.4f TestKDLoss %.4f Epoch %d (after update) lr %s (Best_Acc1 %.4f @Epoch %d)" % (test_acc, test_acc_top5, cls_loss, kd_loss, epoch, lr, best_acc, best_acc_epoch))
    ...
    try:
       ...
       with open(csv_path,"a") as csvfile: 
        ...
        writer.writerow(["time", "seed", "epoch", "lr", "weight_decay", "dataset", "temperature", "lambdaKD","T_model_name", "best_acc"])
    with open(csv_path, 'a+', newline='') as csvfile:
      # Your CSV writing operations here
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow([datetime.datetime.now(), opt.seed, opt.epoch, opt.lr, opt.weight_decay, opt.dataset, opt.T, opt.lambda_kd, opt.t_model,best_acc.cpu().detach().item()]) 
